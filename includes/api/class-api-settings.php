 ...
 * @subpackage Akz_Wp_React_Kit/Akz_Wp_React_Kit_Api_Settings
 */
...
if (! class_exists('Akz_Wp_React_Kit_Api_Settings')) {
	...
	class Akz_Wp_React_Kit_Api_Settings extends Akz_Wp_React_Kit_Api
	{
		...
		public function update_item($request)
		{
			...
			if (is_wp_error(rest_validate_value_from_schema($params, $schema))) {
				return new WP_Error(
					...
					sprintf(__('The %s property has an invalid stored value, and cannot be updated to null.', 'akz-wp-react-kit'), AKZ_WP_REACT_KIT_OPTION_NAME),
					array('status' => 500)
				);
			}
		...
		/**
		 * Retrieves all of the registered options for the Settings1 API.
		 *
		 * @since 1.0.0
		 *
		...
		protected function get_registered_schema()
		{
			...
			$schema = akz_wp_react_kit_admin()->get_settings_schema();
...
/**
 * Return instance of  Akz_Wp_React_Kit_Api_Settings class
 *
 * @since 1.0.0
 *
 * @return Akz_Wp_React_Kit_Api_Settings
 */
function akz_wp_react_kit_api_settings()
{ //phpcs:ignore
